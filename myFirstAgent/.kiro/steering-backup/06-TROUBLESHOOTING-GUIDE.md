...
**必须遵循 `00-COMMON-GUIDELINES.md` 中的复杂开发流程：**
```
...
| 原则 | 说明 |
|------|------|
| **禁止猜测** | 不能根据文档示例、历史记录推断实际配置 |
| **先确认再改** | 任何代码修改前必须与用户确认 |
| **查实际数据** | 用数据库查询、日志、API 响应验证假设 |
| **不确定就问** | 无法验证时直接询问用户 |
| **用 MCP 前先问** | 使用 mysql_query 前先问用户用哪个数据库 |
...
```
发现 tenantId 定义不一致：
- DDL：NULL DEFAULT ''（注释：系统管理员角色为NULL）
- 数据库：NOT NULL DEFAULT ''

初步分析：
1. 业务角度：SystemAdmin 是跨租户的，分配角色时不应绑定特定租户 → 支持 NULL
2. 代码角度：TenantIgnoreContext.runIgnoredIf(isSystemAdmin...) 说明代码预期可以为空 → 支持 NULL
3. 注释角度：DDL 注释明确写"系统管理员角色为NULL" → 支持 NULL

建议：数据库可能被错误修改，建议恢复为 NULL
```
...
| 数据不更新/显示旧数据 | 浏览器缓存 | URL 加 `?nocache=1` |
...
**连接信息**（示例）：
- 地址：`http://<nacos-host>:8848/nacos`
- Namespace：根据环境选择（dev/staging/prod）

**关键配置文件**：
- `shared-*.yml` - 共享配置（Redis、数据库等）
- `iam-lite-app-*.yml` - IAM 服务专属配置
- `gateway-app-*.yml` - Gateway 专属配置

---

## 前端部署问题

**典型场景**：API 路径缺少前缀导致 404

**排查步骤**：
1. 浏览器 Network 面板查看实际请求路径
2. 对比本地代码中的 API 定义
3. 确认线上部署的是最新代码
