...
# 单元测试编写规范（Unit Test Guidelines）

## 概述

本规范定义了单元测试的编写标准，确保测试覆盖完整、可维护。
...
class QuantityTest {
    ...
    void add_twoQuantities_returnsNewQuantity() {
        ...
        // Verify immutability
        assertEquals(BigDecimal.TEN, q1.value());
    }
    
    @Test
    void equals_sameValue_returnsTrue() {
        Quantity q1 = Quantity.of(BigDecimal.TEN);
        Quantity q2 = Quantity.of(BigDecimal.TEN);
...
class InboundOrderTest {
    ...
    @Test
    void create_validInput_createsOrderWithDraftStatus() {
        InboundOrder order = InboundOrder.create(
            SupplierId.of("supplier-1"),
            OrderNo.generate()
        );
        
        assertNotNull(order.getId());
        assertEquals(InboundStatus.DRAFT, order.getStatus());
    ...
    @Test
    void confirm_nonDraftOrder_throwsException() {
        InboundOrder order = createConfirmedOrder();
        
        assertThrows(InvalidStatusException.class, () -> order.confirm());
    }
}
```
...
class InventoryDomainServiceTest {
    ...
    @Test
    void transfer_insufficientInventory_throwsException() {
        Inventory from = createInventory(10);
        Inventory to = createInventory(50);
        Quantity quantity = Quantity.of(BigDecimal.valueOf(30));
...
class InboundOrderApplicationServiceTest {
    ...
    @InjectMocks
    private InboundOrderApplicationService service;
    
    @Test
    void createOrder_validCommand_createsAndSavesOrder() {
        CreateInboundOrderCommand command = new CreateInboundOrderCommand(
            "supplier-1",
            List.of(new LineCommand("item-1", 100))
        );
        
        InboundOrderId result = service.createOrder(command);
        
        assertNotNull(result);
        verify(repository).save(any(InboundOrder.class));
    }
    
    @Test
    void confirmOrder_existingOrder_confirmsAndPublishesEvent() {
        InboundOrder order = createOrderWithOneLine();
        ...
        verify(eventPublisher).publish(any(InboundOrderConfirmedEvent.class));
    }
}
```
