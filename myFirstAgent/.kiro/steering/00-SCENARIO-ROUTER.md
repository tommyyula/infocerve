...
4. 复杂任务用 task-manager - 思考(research_mode)、拆分(split)、执行(execute)、验证(verify)
   - 如 task-manager 不可用（例如 Transport closed），切换 shrimp-task-manager 作为备选执行
...
3. 严格按 12 阶段门禁执行：
   ...
   - 阶段 4-6：设计（domain-model -> demo -> prds）
   - 阶段 7-8：技术设计（design -> repository-design）
   - 阶段 9-11：测试用例设计（e2e -> unit -> api）
   - 阶段 12：实现任务
...
- 阶段 6（FSD 文档）：#28-FSD-GUIDELINES
- 阶段 7（技术设计）：#26-DESIGN-GUIDELINES
- 阶段 8（仓储设计）：#27-REPOSITORY-DESIGN
- 阶段 9-11（测试用例）：#30-TEST-CASE-GUIDELINES
- 阶段 12（实现）：#10-JAVA-CHECKLIST + #31-UNIT-TEST-GUIDELINES
